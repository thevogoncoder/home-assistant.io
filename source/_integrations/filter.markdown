...
filters:
  ...
  keys:
    ...
    offset:
      description: See [_offset_](#offset) filter. Constant offset to apply
      required: false
      type: float
      default: 0
{% endconfiguration %}
...
The Offset filter (`offset`) is a very basic filter, as it adds a constant offset (positive or negative) to a value.
